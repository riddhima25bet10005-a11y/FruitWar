...
if not URL or not KEY:
    supabase = None
    print("WARNING: SUPABASE_URL or SUPABASE_KEY not set. Backend will fail until configured.")
else:
    supabase = create_client(URL, KEY)

def check_db():
    if not supabase:
        raise HTTPException(status_code=500, detail="Database not configured. Please set SUPABASE_URL and SUPABASE_KEY.")
